...
describe('Single test', () => {
    ...
    it('search Senior QA Engineer when filter by engineering and mexico', () => {
        ...
        cy.get('[id="select2-drop"]').find('[class="select2-result-label"]').contains('Engineering').click()
        cy.get(job_openings_page.office_select).click()
        cy.get('[id="select2-drop"]').find('[class="select2-result-label"]').contains('Mexico').click()
        cy.get(job_openings_page.job_container).filter(':visible').contains('Sr. QA Engineer').should('exist')
    })
